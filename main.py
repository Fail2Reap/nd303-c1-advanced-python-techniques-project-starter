...
commands without having to wait to reload the database each time. However, it
doesn't hot-reload.
...
def make_parser():
    ...
    query = subparsers.add_parser(
        'query',
        description=("Query for close approaches that match a"
    ...
    filters.add_argument(
        ...
        help="In kilometers per second. Only return close approaches"
             " whose relative velocity to Earth at approach is as fast or"
             " faster than the given velocity.")
    filters.add_argument(
        '--max-velocity', dest='velocity_max', type=float,
        help="In kilometers per second. Only return close approaches"
             " whose relative velocity to Earth at approach is as slow or"
             " slower than the given velocity.")
    filters.add_argument(
...
class NEOShell(cmd.Cmd):
    ...
    def do_query(self, arg):
        ...
        command line. For example, to query close approaches on January
        1st, 2020:
    ...
    def precmd(self, line):
        ...
        if changed:
            changed_files = ', '. \
                join(str(f.relative_to(PROJECT_ROOT)) for f in changed)
